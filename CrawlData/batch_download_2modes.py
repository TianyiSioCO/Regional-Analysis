...
Supports two modes:
  ...
  2. PSM single-year mode: Download actual data for a specified year (consistent timeline for all points)

Uses polygon to define boundaries, generates grid coordinates, and downloads data point by point
"""
...
# Data mode selection
# - "tmy": Download TMY typical meteorological year data (default, but years may vary)
...
# Available years: 1998-2023 (Continental US)
SINGLE_YEAR = 2020  # Recommend selecting a meteorologically normal year
...
# Multiple API Key configuration - supports rotation across multiple credentials
...
COOLDOWN_DURATION = 60  # Cooldown time per key (seconds)
ALL_KEYS_COOLDOWN_WAIT = 30  # Wait time when all keys are on cooldown (seconds)

# Alaska simplified boundary polygon vertices (longitude, latitude) - mainland portion
# Note: Alaska is extremely large, recommend increasing grid resolution or limiting download points
AK_POLYGON = [
    ...
    (-139.0, 59.5),
    (-140.5, 60.0),
    # Eastern boundary (Canadian border at -141° meridian)
    (-141.0, 60.0),
    (-141.0, 62.0),
    (-141.0, 64.0),
    (-141.0, 66.0),
...
DOWNLOAD_INITIAL_WAIT = 10  # Initial download wait time (seconds) - can be shorter with multiple keys
DOWNLOAD_RETRY_WAIT = 3  # Retry wait time (seconds) - quick polling
MAX_DOWNLOAD_RETRIES = 100  # Maximum retry attempts - increased due to shorter wait time
DOWNLOAD_THREADS = 10  # Parallel download threads - higher concurrency supported with multiple keys
# ==================================
...
class ApiKeyManager:
    ...
    - Supports rotation across multiple API keys
    - Automatically switches and marks cooldown on 429/400 errors
    - Waits and retries when all keys are on cooldown
    """
    ...
    def _get_available_credential(self):
        """Internal method: get available credential, wait if all are on cooldown"""
        while True:
            now = time.time()
            available_indices = []
            ...
            if available_indices:
                # Prefer current index, use first available if unavailable
                ...
                print(f"\n⚠️  All API keys are on cooldown, waiting {wait_time:.0f} seconds before retry...")
                self.lock.release()
                time.sleep(wait_time)
                self.lock.acquire()

    def mark_cooldown(self, index):
        """Mark specified key as on cooldown"""
        with self.lock:
    ...
    def switch_to_next(self, silent=True):
        ...
        silent: Silent mode, don't print switch info (used for normal rotation)
        """
        with self.lock:
...
def generate_grid_points_in_polygon(polygon, resolution, max_points=None):
    ...
    max_points: Maximum point limit, None for no limit (generates all points within polygon)
    """
...
def get_api_config():
    """
    Returns corresponding API endpoint and names parameter based on DATA_MODE
    """
...
def request_download_link(lat, lon):
    ...
    while True:  # Continuously retry until successful
        ...
        # CSV direct download uses GET request with parameters in URL
        params = {
            "api_key": api_key,
            "wkt": f"POINT({lon} {lat})",
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Accept csv, text, octet-stream formats
                # Some environments return binary/octet-stream but actual content is CSV
                if "csv" in content_type or "text" in content_type or "octet-stream" in content_type:
                    # Try to get content
                    try:
                        # If octet-stream, try decoding as text
                        if "octet-stream" in content_type:
                            content = response.content.decode("utf-8")
                        else:
                            content = response.text

                        # Validate if it's valid CSV format (check for comma-separated data rows)
                    ...
                    # Returned non-CSV content, may be an error
...
def download_and_extract(item, output_dir, print_lock):
    """
    Real-time detection and download/extract single file
    """
...
def main():
    ...
    if DATA_MODE == "single_year":
        ...
        print("  ✓ All points use the same year, consistent timeline")
    else:
    ...
    print(
        f"Maximum download points: {'No limit (actual range)' if MAX_POINTS is None else MAX_POINTS}"
    )
    ...
    print(f"API key count: {len(API_CREDENTIALS)}")
    ...
    print("Generating grid coordinate points (within New York State polygon only)...")
    ...
    print(f"\nTotal of {len(csv_files)} CSV files in directory")
